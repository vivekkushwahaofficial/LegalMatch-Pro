...
import { Search, MapPin, Heart, CheckCircle, ShieldCheck } from 'lucide-react';
...
const NgoDirectory = () => {
    ...
    useEffect(() => {
        const fetchNgos = async () => {
            try {
                // Assuming an endpoint that returns all NGOs
                const response = await axios.get('/api/directory/ngos');
    ...
    const filteredNgos = ngos.filter(ngo => 
        ...
        ngo.expertise.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            {loading ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredNgos.map((ngo) => (
                        <div key={ngo.id} className="bg-white rounded-[40px] border border-gray-100 p-8 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all group">
                            ...
                            <p className="text-pink-600 font-bold text-sm uppercase tracking-wider mb-6">{ngo.expertise}</p>
